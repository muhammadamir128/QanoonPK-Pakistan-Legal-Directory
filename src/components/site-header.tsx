  ...
  return (
    <>
      <header className="sticky top-0 z-50 w-full border-b border-border/70 bg-background/85 backdrop-blur-md supports-[backdrop-filter]:bg-background/70">
        <div className="container mx-auto max-w-[1440px] px-3 sm:px-6">
          <div className="flex h-16 items-center justify-between gap-2 xl:gap-3">
            ...
            <nav className="hidden lg:flex items-center gap-0.5 xl:gap-1 shrink-0">
              {navItems.map((item) => {
                const isFinder = item.href === '/finder'
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                  ...
                  >
                    {isFinder && <Compass className="h-3.5 w-3.5 text-primary shrink-0" />}
            ...
            {/* Search (desktop) */}
            <form onSubmit={onSearch} className="hidden md:flex relative items-center shrink-0">
              <Search className="absolute left-2.5 h-3.5 w-3.5 text-muted-foreground pointer-events-none" />
              <Input type="search" placeholder={t('Search laws...', 'قوانین تلاش کریں...')}
                value={q}
                onChange={(e) => setQ(e.target.value)}
                className="pl-8 pr-3 h-9 text-xs transition-all duration-200 w-32 focus:w-48 lg:w-32 lg:focus:w-48 xl:w-44 xl:focus:w-60 2xl:w-56 bg-muted/40 border-border/60 focus-visible:ring-1 focus-visible:ring-primary rounded-lg"
              />
            </form>
